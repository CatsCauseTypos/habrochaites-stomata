...
class(data$plant.number) #default is sometimes as character, so definitely need that extra "as.factor" in there!
...
# Stomata scatter plot without key
ggplot(data3, aes( x = norm.ab.stom, y = norm.ad.stom, colour = Sig.stom )) +
  geom_jitter()+
  geom_point(size=3.5)+
  theme_bw()+
...
abaxial$index <- (abaxial$Stomata/abaxial$Pavement)
head(abaxial)
adaxial$index <- (adaxial$Stomata/adaxial$Pavement)

# abaxial

ggplot(abaxial, aes(reorder(Geno,index, mean), index, fill=Parent))+
  theme_bw()+
...
# adaxial
ggplot(adaxial, aes(reorder(Geno,index, mean), index, fill=Parent))+
  theme_bw()+
  stat_summary(fun.data=plot.summary, geom = "bar", aes(width=.8))+
  stat_summary(fun.data=plot.summary, geom = "errorbar", width=.4)+
  scale_fill_manual(name="Color Key", values = c("darkorange", "darkblue","magenta"))+
...
# # when I use 'na omit' I get no more warnings
