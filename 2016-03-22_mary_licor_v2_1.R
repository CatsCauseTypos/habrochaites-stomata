...
# switching genotype to be colored and treatment to be shape!
ggplot(data, aes( Tleaf, Photo, colour = Treat, shape = Geno))+
  geom_jitter(size = 3.2)+
  xlab("Leaf Temp")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Photosynthesis & Leaf Temperature \nDuring Treatment")+
  theme(legend.position = c(.91,.75))+
...
ggplot(data1, aes(Geno, Photo, colour = Geno))+
  geom_boxplot()+
  facet_grid(~Treat)+
  scale_color_manual(values = c("seagreen3","plum"))+
  ggtitle("Photosynthetic Rates under Stress (Trial B)")+
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
...
# conductance to CO2
ggplot(data_all, aes(y = CndCO2,x = Treat, colour = Geno))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno)+
  #scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
...
# trying to rename the stupid facets 

# some of the internet thinks you have to make a string... no matter what length we make it, we get an error: Error in cbind(labels = list(), list(`{`, if (!is.null(.rows) || !is.null(.cols)) { : number of rows of matrices must match (see arg 2)
fa_names <- c("a")
fa_names <- c("a", "b")
fa_names <- c("a", "b", "c")
fa_names <- c("a", "b", "c", "d")
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno, labeller = label_value(a b c d, multi_line = TRUE))+  # unexpected symbol
  #scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature")+
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno, labeller = label_value("a" "b" "c" "d", multi_line = TRUE))+ # unexptected string
  scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature")+
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno, labeller = label_value(c("a" "b" "c" "d"), multi_line = TRUE))+ # unexpected string constant 
  scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature")+
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno, labeller = labeller(.rows = c("Cont", "WTF"), .cols = c("fak", "u")))+ #plots AND fucks up the labels, but fails to actually rename them...
  scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature")+
...
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 3.5)+
  facet_grid(Treat~Geno, labeller = label_value(, fa_names))+ #plots AND fucks up the labels, but fails to actually rename them...
  scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature")+
...
fa_names <- as.factor(c("a","b")) # nope, not that either

# Below, find my ideas stolen from: http://docs.ggplot2.org/current/labeller.html
# Interactions w labels ----
ggplot(data_all, aes(Cond, Photo, colour = Tleaf))+
  geom_jitter(size = 5)+
  facet_grid(Treat~Geno, labeller = labeller(
    Geno = c(
      e6203 = "Domesticated",
      hab = "Wild"
    ),
...
# after
ggplot(data_all, aes(Cond_after, Photo_after, colour = Tleaf_after))+
  geom_jitter(size = 5)+
  facet_grid(Treat~Geno, labeller = labeller(
    Geno = c(
      e6203 = "Domesticated",
  ...
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  coord_cartesian(ylim = c(-4,14))+ # cannot get this to work ><
  coord_cartesian(xlim = c(0,.4))+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature Recovered")+
  theme(plot.title = element_text(size = 15.2, face="bold"))

# shape as treatment ----
# after
ggplot(data_all, aes(Cond_after, Photo_after, colour = Tleaf_after, shape = Treat_after))+
  geom_jitter(size = 5)+
  facet_grid(~Geno, labeller = labeller(
    Geno = c(
      e6203 = "Domesticated",
      hab = "Wild"
    )
  ...
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  coord_cartesian(ylim = c(-5,15))+ # cannot get this to work ><
  coord_cartesian(xlim = c(0,.4))+
  ggtitle("Interactions of Conductance, Photosynthesis, & Leaflet Surface Temperature Recovered")+
  theme(plot.title = element_text(size = 15.2, face="bold"))

# during stress
ggplot(data_all, aes(x = Cond, Photo, colour = Tleaf, shape = Treat))+
  geom_jitter(size = 5)+
  facet_grid(~Geno, labeller = labeller(
    Geno = c(
      e6203 = "Domesticated",
...
# # rearranged data file with two different "treatment" styles 
# 
 data_etf <- read.csv("~/Desktop/daw_amalgamation_mary_trimmed2.csv")
# 
# # Truly all data
# ggplot(data_etf, aes(x = Cond, Photo, colour = Tleaf, shape = Treat))+
#   geom_jitter(size = 3.5)+
#   facet_grid(~Geno, labeller = labeller(
#     Geno = c(
#       e6203 = "Domesticated",
...
#   xlab("Leaflet Water Conductance")+
#   ylab("Photosynthetic Rate (A)")+
#   scale_shape_manual(name = "Treatment & Time Point",
#                      labels = c("Normal Water During", "No Water During", "Normal Water Recovered", "No Water Recovered"),
...
#   theme(plot.title = element_text(size = 15.2, face="bold"))
# 
# # we agree that it's a good attempt at combining all the data things, but dang is it hard to understand wtf is happening as a reader.

## ok, so I still can't get ylim to work, let's try something different:

# graphing interactions w/ species separately ----

# below, mapping all of the photo/conduct/tleaf shows the TRUE photosynthetic curve we expect in all plants (though maybe our rates are a bit low for tomato...) - unexpected and undesired graph, but cool. 
data_etf %>% 
  group_by(Treat) %>% 
ggplot(., aes(x = Cond, Photo, colour = Tleaf, shape = Geno))+
  geom_jitter(size = 5)+
  stat_smooth(color = "pink", se = FALSE )+ # I would like to color these by geno, but it's not working
  scale_colour_gradient( low="yellow", high = "darkorchid", space = "Lab")+
  xlab("Leaflet Water Conductance")+
  ylab("Photosynthetic Rate (A)")+
  coord_cartesian(ylim = c(-5,15))+ 
