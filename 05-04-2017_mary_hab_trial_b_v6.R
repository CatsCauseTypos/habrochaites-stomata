...
# scatter plot adaxial to see data ----
...
# scatter plot abaxial ----
ggplot(Trial_b, aes( x = ab_pave, y = ab_stomata, colour = Treatment, shape = Geno) ) +
  geom_point(size=3.5)+
  theme_bw()+ # this left-justifies title
  scale_color_manual(name="Color Key:", values= c( "darkblue", "darkgoldenrod3"),
                     breaks = c("00_norm","none"),
...
ggplot(Trial_b, aes(reorder(Geno,ad_pave, mean), ad_pave, fill = Treatment))+
  ...
  ylab("Average Adaxial\nPavement Cells")+
  xlab("Species")+
  coord_cartesian(ylim = c(0, 220))

# bar plot stomatal index adaxial ---- needs help
...
ggplot(Trial_b, aes(reorder(Treatment), y = Index_ad, x = Geno, fill = Treatment))+
  ...
  scale_fill_manual(values= c( "darkblue", "darkgoldenrod3"),
                    breaks = c("00_norm", "none"),
                    labels = c("Normal Watering", "No Water"))+
  theme(plot.title = element_text(lineheight=.8,hjust = 0.5, face="bold", size = 28),
  ...
  ylab("Adaxial Stomata Pavement Cells")+
  xlab("Genotypes")+
...
# hey, what if I run an FDR-adjusted instead of tukey?
trying <- lm(Index_ad~Treatment*Geno, Trial_b)
sum_trying<- as.data.frame(summary(trying)$coefficients[,4])
try.adj.p <- data.frame(value=p.adjust(summary(trying)$coefficients[,4], method="fdr")) 
View(try.adj.p)
try.adj.p$Geno=rownames(try.adj.p) #this makes life easier later
try.sigf.p <- as.data.frame(try.adj.p[which(try.adj.p$value<.05),])
try.sigf.p # something is wrong with this. why do i never see hab?? 
...
ggplot(Trial_b, aes(reorder(Geno,ab_pave, mean), ab_pave, fill = Treatment))+
  ...
  scale_fill_manual(values= c( "darkblue", "darkgoldenrod3"),
                    breaks = c("A Normal Watering", "No Water"),
                    labels = c("Normal Watering", "No Water"))+
  theme(legend.position = c(.138,.869),
